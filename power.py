...
def isfabric_cluster():
    """Checks if the system is a fabric cluster"""
    return os.path.isdir(os.path.join(UNCORE_PATH, "uncore00"))


def show_available_uncore_clusters():
    """Display fabric clusters available in the system"""
    if isfabric_cluster():
        cluster_list = []
...
def get_cluster_id_mapping():
    """Returns dictionary with fabric clusters and it package ID as key, value pairs """
    cluster_id_mapping = {}
...
def fabricinfo():
    """This function collects and prints uncore fabric cluster information"""
...
def set_uncore_fabric_sysfs(uncore_freq, fabric_cluster, path_constant):
    """Set uncore_ freq for the fabric cluster using sysfs."""
    cluster_info = "uncore" + fabric_cluster
...
def set_uncore_min_fabric_sysfs(uncore_freq, fabric_cluster):
    """Set uncore_ freq as min freq for the fabric cluster using sysfs."""
    set_uncore_fabric_sysfs(uncore_freq, fabric_cluster, UNCORE_MIN)


def set_uncore_max_fabric_sysfs(uncore_freq, fabric_cluster):
    """Set uncore_ freq as max freq for the fabric cluster using sysfs."""
    set_uncore_fabric_sysfs(uncore_freq, fabric_cluster, UNCORE_MAX)


def get_fabric_cluster_from_user(min_freq, max_freq):
    """With a valid user input the function sets min/max frequency"""
    show_available_uncore_clusters()
    pattern = r'^\d{2}$'
    fabric_cluster = raw_input("Select uncore cluster:")
    try:
        if re.match(pattern, fabric_cluster):
            if min_freq:
                set_uncore_min_fabric_sysfs(min_freq, fabric_cluster)
            elif max_freq:
                set_uncore_max_fabric_sysfs(max_freq, fabric_cluster)
        else:
            raise ValueError()
...
def set_uncore_freq(min_freq=None, max_freq=None):
    if (isfabric_cluster()):
        # Set Uncore for specific fabric cluster
        print("\nUpdating uncore frequency...")
        get_fabric_cluster_from_user(min_freq, max_freq)
    else:
        directory = UNCORE_PATH[0:-1]
        if not os.path.exists(directory):
            print(f"Missing [{directory}], is kernel uncore driver installed?")
...
def show_help():
    ...
    print('   -f            Show UNCORE fabric information')
    ...
    print()
    print('4. Display information on the uncore fabric')
    print()
    print('   ' + scriptname + ' -f')
    print()
...
def do_menu(cpurange):
    ...
    print("")
    print("[10] Display Uncore Fabric Information")
